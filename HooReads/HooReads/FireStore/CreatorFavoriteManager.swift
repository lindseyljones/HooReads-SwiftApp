...
final class CreatorFavoriteManager {
    static let shared = CreatorFavoriteManager()
    private init() {}
    
    func createCreatorFavs() async throws {
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book.bookId).setData(BookData, merge: false)
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book2.bookId).setData(Book2Data, merge: false)
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book3.bookId).setData(Book3Data, merge: false)
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book4.bookId).setData(Book4Data, merge: false)
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book5.bookId).setData(Book5Data, merge: false)
        var book6 = try await BookAPIManager.shared.getBook(name: "Carrie Soto Is Back")
        ...
        try await Firestore.firestore().collection("creator_favorites").document(book6.bookId).setData(Book6Data, merge: false)
        // the maidens
        // taylow jenkins reid
        // kristin hannah
    ...
    func getCreatorFavs() async throws -> [BookAPIManager.Book] {
        print("get creator FAVs")
        let snapshot = try await Firestore.firestore().collection("creator_favorites").getDocuments()
